...
if [ ! -f "$RELEASE_DIR/dist/image-worker.js" ]; then
  echo "图片 Worker production build 不存在: $RELEASE_DIR/dist/image-worker.js" >&2
  exit 1
fi
if [ ! -f "$ENV_FILE" ]; then
...
if [ -n "${DIRECT_DATABASE_URL:-}" ]; then
  echo "Worker 不得继承数据库 owner 直连凭据" >&2
  exit 1
fi
...
cd "$RELEASE_DIR"
exec env NODE_ENV=production BUILD_SHA="$BUILD_SHA" "$NODE_BINARY" dist/image-worker
