...
const signatures = new Set(
  fixture.markSets.map((item: { marks: object }) => Object.keys(item.marks).sort().join(',')),
);
...
assert.equal(
  new Set(fixture.cases.map((item: { id: string }) => item.id)).size,
...
for (const item of fixture.cases) {
  if (item.kind === 'canonical') {
    ...
    assert.deepEqual(
      summarize(item.canonical).blocks,
      [
        {
          type: 'paragraph',
          alignment: 'left',
          children: item.segments.map((segment: object) => ({ type: 'text', ...segment })),
        },
      ],
      item.id,
    );
    ...
    const length = item.segments.reduce(
    ...
    assert.equal(
      item.segments.map((s: { text: string }) => s.text).join(''),
      item.expectedSegments.map((s: { text: string }) => s.text).join(''),
      item.id,
    );
  }
}
...
const old = JSON.parse(
  readFileSync('contracts/markdown-editor-roundtrip-v7-fixtures.json', 'utf8'),
);
...
for (const client of ['wenyousite-frontend', 'wenyousite-mobile']) {
  for (const [filename, expected] of [
    ...
    if (existsSync(path))
      assert.equal(readFileSync(path, 'utf8'), expected, `${client}/${filename} 漂移`);
  }
}
console.log(
  `Inline combinations contract passed: 32 mark sets, 1024 ordered pairs, ${fixture.cases.length} explicit cases`,
);
