...
const fixturePath = 'contracts/editor-clipboard-v2-fixtures.json';
...
if (
  fixture.contract !== 'wenyousite-editor-clipboard' ||
  fixture.version !== 2 ||
...
if (
  ...
  envelope.versionValue !== '2' ||
  !Array.isArray(envelope.acceptedVersionValues) ||
  !['1', '2'].every((version) => envelope.acceptedVersionValues.includes(version)) ||
  envelope.sourceAttribute !== 'data-wenyou-clipboard-source' ||
  envelope.blockAlignmentAttribute !== 'data-wenyou-align' ||
  JSON.stringify(envelope.blockAlignmentValues) !== JSON.stringify(['center', 'right']) ||
  envelope.validation !== 'strict-allowlist' ||
  envelope.authentication !== 'none'
) {
  failures.push('Web HTML envelope 必须写入 v2、兼容读取 v1，并严格白名单校验对齐属性');
}
...
const entryPoints = uniqueIds(
  Array.isArray(fixture.entryPoints) ? fixture.entryPoints : [],
  'entryPoints',
);
...
const actualPasteRules = new Set(
  pasteRules.map((item) => `${String(item.source)}:${String(item.result)}`),
);
for (const required of [
  'site-fragment-v1:structured-without-alignment',
  'site-fragment-v2:structured',
...
if (
  diceRule?.paste !== 'regenerate-node-id-unrolled' ||
  diceRule?.cutPaste !== 'preserve-node-id'
) {
  failures.push('骰子必须复制换 ID、剪切保留 ID，且不得继承投掷结果');
}

const alignmentRule = isObject(fixture.alignmentRule) ? fixture.alignmentRule : {};
if (
  JSON.stringify(alignmentRule.eligibleBlocks) !==
    JSON.stringify(['paragraph', 'heading-2', 'heading-3']) ||
  JSON.stringify(alignmentRule.excludedBlocks) !==
    JSON.stringify(['bullet-list', 'ordered-list', 'blockquote', 'image', 'horizontal-rule']) ||
  alignmentRule.readerCopy !== 'preserve' ||
  alignmentRule.editorCopy !== 'preserve' ||
  alignmentRule.sitePaste !== 'preserve' ||
  alignmentRule.externalPaste !== 'literal-text' ||
  alignmentRule.plainText !== 'omit'
) {
  failures.push('对齐剪贴板规则必须只保留段落与 H2/H3，外部来源和纯文本不得恢复对齐');
}

const goldenCases = uniqueIds(
  Array.isArray(fixture.goldenCases) ? fixture.goldenCases : [],
  'goldenCases',
);
...
const settledDiceCase = goldenCases.find(
...
if (
  ...
  !settledDiceCase.expectedAtoms.some(
    (atom) =>
      isObject(atom) &&
      atom.rollState === 'discard-result-on-paste' &&
      atom.nodeId === 'regenerate',
  )
) {
  failures.push('黄金用例必须固定已结算骰子的可见结果，并在粘贴时丢弃结果和旧身份');
}

const alignedCase = goldenCases.find((item) => item.id === 'reader-aligned-blocks');
if (
  !alignedCase ||
  alignedCase.expectedPlainText !== '居中标题\n\n居右正文' ||
  JSON.stringify(alignedCase.expectedBlockAlignments) !== JSON.stringify(['center', 'right'])
) {
  failures.push('黄金用例必须固定阅读态对齐结构复制，并从纯文本回退中移除协议标记');
}
...
for (const sibling of ['../wenyousite-frontend', '../wenyousite-mobile']) {
  ...
  if (fs.existsSync(siblingFixture) && fs.readFileSync(siblingFixture, 'utf8') !== fixtureSource) {
    failures.push(`${path.basename(sibling)} 的剪贴板 v2 fixture 与事实源不一致`);
  }
}
...
console.log(
  `Editor clipboard contract is valid (${entryPoints.length} entry points, ${goldenCases.length} golden cases)`,
);
