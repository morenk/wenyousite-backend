...
创建主题帖、子贴、楼层/回复、私聊消息、表情导入和温油打赏都使用 UUID v4 `clientRequestId`。一次用户操作从首次发送到所有超时重试必须复用同一个值；用户明确再次点击“新建”才生成新值。

每日签到由已登录客户端启动时调用 `POST /wallet/check-in`；服务端按北京时间自然日幂等处理。只有 `claimedNow=true` 时展示本次 1～3 升领取提示。金额字段均为十进制整数字符串，客户端不得转为浮点数；打赏最低 2 升，收款人到账 85% 向下取整，平台保留余数。
