...
- 主题帖、楼层和回复使用 Markdown v4 工具栏能力白名单。客户端必须消费 [`markdown-v4-fixtures.json`](../contracts/markdown-v4-fixtures.json)、[`markdown-v4-nodes-fixtures.json`](../contracts/markdown-v4-nodes-fixtures.json)、[`markdown-editor-roundtrip-v6-fixtures.json`](../contracts/markdown-editor-roundtrip-v6-fixtures.json) 与 [`editor-clipboard-v2-fixtures.json`](../contracts/editor-clipboard-v2-fixtures.json)，覆盖规范化、允许/拒绝、字面文本降级、扩展节点、普通软换行、块语义、块对齐、行内边界语义和剪贴板 round-trip；第三方解析器支持的表格等语法不得自行扩大产品能力。v6 继续保留 Setext H2/分隔线和行内定界符边界规则；写回仍以字符引用保护相邻正文、不增加可见空格，并把下划线定界符规范为星号。
- Windows 移动端必须固定 Foundation `v6.7.0`，把 `align` 加入 Delta 行属性白名单，并让 Markdown→Delta Codec 只为普通段落与 H2/H3 解码、编码紧邻的 `center|right` 标记；左对齐删除标记。工具栏以单一“对齐”入口在更多面板切换左/中/右，阅读器按块应用一致排版；列表、引用、普通图片、分隔线和协议空段不得继承属性，提及、骰子和收藏表情随父段落。已审查提交 `6a14e84a3cd58ed7705e645a4e590e52ab57f2e2` 同时接受服务端版本 3 与 4，`/meta.markdownContractVersion` 现已激活为 4。
