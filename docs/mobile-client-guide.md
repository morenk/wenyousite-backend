...
### 文档级空块组合（列表 revision 3）

从本候选最新固定提交同步列表 fixture revision 3。新增 `documentCases.tree` 描述完整文档块树，`operations` 顺序替换 `path` 指向文字块的 `text`；每步预期由独立树施加指定替换，其他节点、空段和编号保持。核对读取、结构剪贴板、真实编辑、保存重开和可发布空状态，不能只比较列表数量或同一 Codec 自洽。48 条两组列表矩阵覆盖两种类型的全部组合、每组末项空/非空与一至三个明确空段；另覆盖列表尾空段、空 H2/H3、空引用。语法分隔不算实际空行，自动输入占位不能替代作者空段。

`visible=false` 表示只有空结构、不可单独发布；结构仍须可编辑、可保存为草稿。结构剪贴板保留空块；纯文本回退继续遵循 clipboard v2 已有换行压缩，不将其当成富文本保真载体。此扩展不修改共享语义；如客户端无法无损承载，保留原内容并报告具体差异。
