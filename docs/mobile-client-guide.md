...
- 主题帖、楼层和回复使用 Markdown v3 工具栏能力白名单。客户端必须消费 [`markdown-v3-fixtures.json`](../contracts/markdown-v3-fixtures.json)、[`markdown-v3-nodes-fixtures.json`](../contracts/markdown-v3-nodes-fixtures.json) 与 [`markdown-editor-roundtrip-v4-fixtures.json`](../contracts/markdown-editor-roundtrip-v4-fixtures.json)，覆盖规范化、允许/拒绝、字面文本降级、扩展节点、普通软换行和块语义 round-trip；第三方解析器支持的表格等语法不得自行扩大产品能力。同步 v4 时必须先把 `正文\n---` 识别为 Setext H2 并写回 `## 正文`，只有与前后正文各隔一个空行的 `正文\n\n---\n\n正文` 才表示分隔线。
