...
- 摘要先把字面 `<` 用私有区占位符保护再交给 remove-markdown，避免跨行的 `<...>` 被误判为 HTML 标签整段吞掉；清理后还原 `<` 与 Milkdown 转义（`\>` → `>`、`\<` → `<` 等），因此字面 `<`/`>` 保留展示、不残留孤立 `\`；普通反斜杠路径（如 `C:\temp`）不受影响
- 真实引用块（Markdown 的 `> 文本`）仍被清理，保留引用内容；Milkdown 转义的字面 `>`（存为 `\>`）还原为 `>` 展示，不会被当作引用整行剥掉（否则回复内容只剩 `>` 时预览会变空）
