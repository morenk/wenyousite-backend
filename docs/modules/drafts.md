...
| 模型 | 用途 |
|------|------|
| `Draft` | 草稿实体（userId + slot 联合唯一，content + pendingDiceNotations 快照，version 乐观锁） |
...
- 删除为硬删除（物理删除），不使用软删除
- 草稿把 `content` 与 `pendingDiceNotations` 作为同一个版本快照原子保存；允许纯正文或纯待掷骰子，但两者不能同时为空
- 待掷表达式使用基础骰子白名单并规范化，最多 20 条；云草稿不生成正式点数
- 创建或更新草稿时统一执行 Markdown v1 规范化，但不执行发布可见性限制
- 多设备继续使用 `version` 做乐观锁；冲突时客户端应停止自动保存、重新拉取远端快照，不做字段级合并
