...
私聊仅支持一对一会话，不追求聊天软件级实时性。所有读写操作要求登录，权限由会话参与者和服务端 capability 决定。

支持纯文本、每条一张图片或一个收藏表情；图片和表情不能与彼此或正文组合发送。新私聊消息可通过 FCM 发送仅含导航 ID 的隐私安全提示，权威消息内容仍由 API 拉取。不支持群聊、输入状态、在线状态、消息编辑、站内搜索、端到端加密或消息举报。
...
- 拉黑不改变会话状态、不删除历史；双方的列表、详情、消息与未读数互相隐藏，新互动被拒绝。双方完全解除后恢复原状态。
- 注销账号的既有历史保留，另一方看到“已注销用户”，会话只读。

## 消息、已读与撤回

- `content` 为最多 1000 字的字符串，保留换行。客户端只额外激活站内传送门与安全 `http/https` 链接，其他 Markdown 保持字面文本；整段粘贴合法站内地址时可写回规范化的标准链接字符串。
...
- 会话列表 `contentPreview` 复用统一传送门预览：普通传送门显示解码后的名称，邀请和残留 `/join/{token}` 显示“邀请传送门”，再规范化空白并截取 120 字符。完整消息与复制操作继续使用原始正文。
...
- 私聊 Outbox、BullMQ/FCM data payload 和推送文案只携带必要的用户、会话、消息 ID 与固定提示，不携带完整正文或邀请 token；不为传送门预抓取目标或建立独立缓存。
