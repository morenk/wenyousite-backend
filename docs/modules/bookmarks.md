...
用户收藏主题帖和动态以便快速返回访问。两类内容共用用户私有收藏夹；每个账号始终有一个“默认收藏夹”，不指定分类的旧客户端和快捷收藏都会落入默认夹。主题帖支持公开帖和私密帖收藏，私密帖仅参与人可收藏。
...
- 收藏夹响应的 `bookmarkCount` 表示主题帖数量，`momentBookmarkCount` 表示动态数量
