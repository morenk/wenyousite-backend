...
- 搜索是用户的显式找帖行为：主题帖与全站正文搜索不因楼主/作者账号注销而隐藏仍然公开的历史内容；只有用户搜索会排除注销账号本身。这些历史帖不会出现在 `GET /threads` 首页发现流。

- 关键词统一 trim，最长 100 个字符；用户名和主题帖允许单字符查询
- 用户搜索：仅匹配用户名，过滤 `deletedAt: null`，只返回 id / username / avatar / bio，最多 20 条并按用户名升序
- 主题帖搜索：仅匹配标题，过滤未发布、已删除和 PRIVATE 主题帖；按标题相关度、创建时间和 ID 稳定排序，返回与首页相同的完整卡片字段及 `meta.cursor/hasMore`
...
- 客户端按 Tab 分别请求分类端点；动态和主题帖搜索都复用各自首页卡片响应与列表组件，默认主题帖不会触发楼层正文查询
- 主题帖搜索保留独立 `/search/threads` 端点，不给 `/threads` 增加关键词参数，避免发现流的推荐缓存、已注销楼主过滤和搜索相关度语义混在同一查询中
