...
主题帖分类以注册表管理：`slug` 创建时先去除首尾空白并转大写，随后必须符合 `^[A-Z][A-Z0-9_]{0,49}$` 且不可修改，作为主题帖外键和 Web/移动端契约中的稳定标识；管理员可随时修改名称、描述、排序和启停状态。分类是纯文本能力，不定义颜色或新图标/合并语义；旧 `icon / mergedIntoId` 仅作存储与线协议兼容。更新分类会失效主题帖列表和详情读缓存，历史主题帖继续通过原 slug 关联并立即显示新的名称。管理列表与公开发现列表都按 `sortOrder ASC, slug ASC` 稳定排序，后者只返回启用项。
