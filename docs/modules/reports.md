...
| `POST` | `/moderation/appeal-token` | 账号密码换取 15 分钟申诉专用凭据；每分钟最多 5 次 |
...
- 未受处罚账号可继续使用普通 Bearer。暂停或封禁账号用账号密码调用 `appeal-token`，把返回值作为 Bearer 调用上述两个申诉接口；该凭据不能访问其他受保护接口、不能刷新，也不创建登录终端。
- 申诉凭据失效返回 `APPEAL_TOKEN_INVALID`。客户端只清理申诉凭据并允许重新验证账号，不把它当作普通会话失效。
