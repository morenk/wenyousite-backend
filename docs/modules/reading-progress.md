...
| `GET` | `/reading-progress/threads/:threadId/new-replies` | `@AuthRead()` | 一次返回主题帖全部子贴的新增回复摘要，避免详情页 N+1 HTTP 请求 |
...
- `findAll` 自动过滤已软删除的子贴
- 帖级聚合先校验主题帖可见性，再批量读取子贴、进度和计数；响应顺序与子贴 `sortOrder` 一致
...
- **详情页批量摘要**：主题帖详情只发一个聚合请求，禁止按子贴循环请求新增回复数
