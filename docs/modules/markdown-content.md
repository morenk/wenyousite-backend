...
- 列表、引用、普通图片、分隔线和协议空段不能携带对齐。含普通图片的段落也不能对齐；提及、骰子和收藏表情是内联原子节点，随合法父段落对齐。
- 孤立标记、标记间空行、重复标记、`left` 标记、未知值或未知版本都按白名单外结构拒绝；需要显示同形源码时必须转义。
- v3 正文不含上述标记，是 v4 的严格子集。后端先接受并保存 v4，但 `/meta.markdownContractVersion` 在旧移动端兼容窗口内仍声明 `3`；Web 只有读取到版本 `4` 后才开放对齐写入。
