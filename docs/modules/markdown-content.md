...
- 当 `* / _ / ~~` 行内定界符包裹的内容以标点或符号开头、结尾，并因紧邻正文而被严格 CommonMark 解析器保留为字面源码时，阅读端和编辑器恢复粗体、斜体、粗斜体或删除线语义；转义定界符、代码、空白边界和普通单词内下划线保持字面文本。编辑器写回时使用字符引用保护相邻正文，不插入可见空格；普通强调优先使用星号，格式区间紧邻时允许选择等价下划线定界符消歧，不能以统一定界符损坏语义。
...
- 相同可见样式的连续文字只构成一个逻辑格式区间；输入来源标记不是可见样式，不应造成 `**旧****新**`。来源仍决定各片段的字面转义，不得为了合并而丢失。规范嵌套由外到内为粗体、斜体、链接、删除线、行内代码；v7 仅覆盖独立行内代码，新增组合以独立行内组合 v1 契约为准。不得跨越换行、原子节点或真正的样式/链接目标变化合并。
...
[组合 fixture](../../contracts/markdown-inline-combinations-v1-fixtures.json) 与 [schema](../../contracts/markdown-inline-combinations-v1.schema.json) 扩充测试契约，保留 roundtrip v7 原有 48 条编辑操作。五个 marks 可组合；行内代码内部始终是字面文字，外层允许粗体、斜体、删除线与安全链接。基础矩阵为 32 个集合（含普通文本）的 1,024 个有序邻接对，并展开无间隔/一空格/两空格、ASCII/中文外侧正文、不同链接目标和特殊字符。`expectedCaseCount` 明确展开总数，不能只验证独立样式而漏掉邻接。
...
摘要在真实 code span 源码边界内保留字面星号、反引号、实体、图片源码和站内 URL，移除代码外层语法定界符；跨行代码按 CommonMark 将内部 LF 变为空格，纯空格代码不误删 padding。紧凑卡片仍按既有规则折叠所有连续空白，普通摘要保留代码可见空格；代码实体不参与普通正文的单遍实体解码。
