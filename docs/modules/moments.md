...
发现/关注流、动态搜索、当前用户收藏和用户主页动态统一返回 `MomentCardResponseDto`，并复用同一 `cardSelect` 与 `mapMomentCard`。搜索只额外附加可选相关度；各入口自己的排序和游标保持独立，登录查看时 `viewerLiked` / `viewerBookmarked` 都按当前查看者计算。
