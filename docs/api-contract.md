...
列表 `preview` 从原始正文识别合法对齐元数据后生成紧凑纯文本；有封面时省略可见图片节点，不重复显示 `[图片]`。首图默认左对齐、居中或居右均不泄漏合法对齐标记；代码、转义及非法或孤立标记保持字面身份。Web 与 Mobile 直接消费同一摘要，无需客户端清理标记或迁移正文；此修复不改变 HTTP 字段、Markdown v5 或封面选择规则。
...
兼容契约 5.26.0-dev.20260922.1 新增 [图片图集查询](image-gallery.md)。Mobile 先显示点击图片，再接入双向分页；Web 保留原查看交互，新增契约只同步类型与夹具。共享位置测试见 `contracts/gallery-image-occurrences.json`，重复 URL 按位置保留，贴纸仅按 title 前缀排除。权限丢失 404 必须清除对应缓存图，40900 重新打开会话，40924 保留当前图片并提示稍后再试。
