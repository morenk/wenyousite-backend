...
游标固定 scope、方向、排序、作者、打开时间以及置顶顺序；打开后新增内容不混入会话。每次续页检查边界图片的版本与身份。会话内任一已存在可见正文更新内容时拒绝续页；置顶变化保持打开时的固定顺序。编辑导致锚点失效返回 HTTP 409 / CONFLICT，提示重新打开；不静默跳到同位置的另一张图。对象不可见返回 404，客户端须清除对应缓存敏感图，不能继续显示；游标格式、会话条件不匹配返回 400 / INVALID_CURSOR。客户端遇失败保留当前图并提供重试；409 要重新加载会话。

## 索引与发布

新增 post_image_occurrences：每个 Post 按普通图片出现位置保存 URL，主键 `(post_id, image_index)`。正文写入与索引替换在同一事务；该索引不替代授权与媒体引用账本。历史通过有界、幂等、可审计的独立回填入口处理，默认 dry-run；线上执行须另行授权。先完成兼容后端迁移、回填与校验，再发布移动端。未回填历史内容返回 HTTP 409 / IMAGE_GALLERY_NOT_READY（40924），不会伪装完整图集。`mediaId` 为已授权绑定的媒体 ID，外链或无法唯一匹配时为 null。签名游标绑定账号与查询条件，24 小时后须重新打开。回填命令 `pnpm exec tsx scripts/backfill-gallery-index.ts --limit=250` 默认只读；经授权加 `--apply`，每批最多 500 行，`--after=<nextAfter>` 可继续，也可从头幂等重跑。
