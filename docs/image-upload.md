...
GIF 以原字节写入正式对象，限制最长边 2560px、最多 300 帧、总时长 60 秒、累计 1 亿像素。`animated=true`，普通用途只生成静态缩略图；正文额外生成独立首帧 poster，并在预算允许时尝试列表动画预览，见[列表动画预览](media-animation-previews.md)。

静态图片用途派生矩阵（GIF 不生成静态 feed/medium；正文 poster 独立于方形 thumbnail）：
