...
发布只增加可空列、新枚举及尝试账目表，必须先通过 owner 执行两项迁移，再启动使用 wenyousite_app 的 API 和图片 Worker；迁移前核验 owner 默认表权限，迁移后核验新表对 app 的 CRUD 权限。部署沿用仓库备份、安全门禁与不可变 release 流程，消费者依赖最终部署 SHA 同步契约。

旧客户端继续读取 coverImages；旧后端在保留新增数据库结构时仍可读写原有字段，但不认识新预览账目和补偿逻辑。因此故障时优先保全日志、停止扩大消费者发布并前滚修复；若确需退回旧应用，必须单独核验队列中任务及预览回收影响，保留新增列、表和已上传对象，不执行逆向删表或覆盖活动数据卷。账目回收能力须随后恢复，应用回退不等于数据或对象回收已经完成。
