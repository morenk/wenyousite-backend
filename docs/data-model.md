...
- `ExperienceEventType`：日活、主题帖/私帖激活、楼层与动态回复、主题帖获赞、动态发布、打赏投入/收款以及未来处罚撤销事件。
...
| profileCoverMediaId | String? | unique, FK → media.id | 个人主页 Web 3:1 背景图；账号注销时置空 |
...
- `daily_check_ins` 以用户和北京时间日期唯一，每日随机领取 1～3 升温油；日活经验可能已由首次有效行为领取，因此 `experience_awarded` 为 0 或 2。

### experience_events / experience_daily_stats — 经验账本

- 正向经验事件按来源幂等写入，日统计按北京时间限制次数；不同互动用户通过幂等键去重，结算只更新用户行、事件和日统计，不扫描历史内容；撤销事件保留原因并可使用户降级。
