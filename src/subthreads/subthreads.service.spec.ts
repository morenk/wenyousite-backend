...
describe('SubthreadsService', () => {
  ...
  describe('update', () => {
    it('默认子贴不可修改 sortOrder', async () => {
      ...
      await expect(service.update('s1', { version: 1, sortOrder: 5 }, 'u1')).rejects.toThrow(
        BusinessException,
      );
    });
  });
  ...
  describe('find', () => {
    ...
    it('findById 应返回子贴详情', async () => {
      ...
      mockPrisma.subthread.findUnique.mockResolvedValue({
        id: 's1',
        threadId: 't1',
        thread: { id: 't1', visibility: 'PUBLIC', ownerId: 'u1', published: true },
      });
