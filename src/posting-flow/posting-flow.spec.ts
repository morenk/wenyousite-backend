...
const createMockPrisma = () => ({
  ...
  postLike: { upsert: jest.fn(), deleteMany: jest.fn(), findUnique: jest.fn() },
...
const setupHelpers = {
  ...
  mockThreadAccess_privateMember: (m: MockPrisma) => {
    m.thread.findUnique.mockResolvedValue({ visibility: 'PRIVATE', published: true, ownerId: 'u1' });
...
describe('发帖全流程集成测试', () => {
  ...
  describe('主题帖草稿 (Thread Draft)', () => {
    it('创建草稿：published=false，自动创建 OWNER 成员', async () => {
      prisma.thread.create.mockResolvedValue({ id: 't1', title: '测试标题', category: 'RPG', visibility: 'PUBLIC', published: false, ownerId: 'u1' });
      ...
      expect(prisma.threadMember.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ role: 'OWNER', playerMarked: true }) }),
      );
    ...
    it('创建草稿：携带标签应同步创建 TopicTag', async () => {
      prisma.thread.create.mockResolvedValue({ id: 't1', title: '测试', category: 'RPG', ownerId: 'u1', published: false });
      prisma.threadMember.create.mockResolvedValue({ id: 'm1' });
      mockTags.findOrCreate.mockResolvedValue([{ id: 'tag1', name: '科幻' }, { id: 'tag2', name: '悬疑' }]);
      prisma.threadTopicTag.createMany.mockResolvedValue({ count: 2 });
      prisma.thread.findUnique.mockResolvedValue({ id: 't1', owner: {}, subthreads: [], topicTags: [], _count: {} });

      await threadsService.create({ title: '测试', category: 'RPG', tagNames: ['科幻', '悬疑'] }, 'u1');
    ...
    it('detail：已发布公开帖增加 viewCount', async () => {
      ...
      expect(result.id).toBe('t1');
      expect(prisma.thread.update).toHaveBeenCalledWith({ where: { id: 't1' }, data: { viewCount: { increment: 1 } } });
    });
    ...
    it('detail：未发布帖 owner 可查看', async () => {
      const thread = { id: 't1', published: false, ownerId: 'u1', visibility: 'PUBLIC', owner: { id: 'u1' }, subthreads: [] };
      prisma.thread.findUnique.mockResolvedValue(thread);
    ...
    it('detail：已发布私密帖非成员返回 404', async () => {
      prisma.thread.findUnique.mockResolvedValue({ id: 't1', published: true, visibility: 'PRIVATE' });
    ...
    it('detail：已发布私密帖成员可查看', async () => {
      const thread = { id: 't1', published: true, visibility: 'PRIVATE', owner: { id: 'u1' }, subthreads: [] };
      prisma.thread.findUnique.mockResolvedValue(thread);
  ...
  describe('发布流程 (Publish)', () => {
    ...
    it('发布时 title 取自 updateData 而非 thread.title', async () => {
      prisma.threadMember.findUnique.mockResolvedValue({ role: 'OWNER' });
      prisma.thread.findUnique.mockResolvedValue({ published: false, title: '旧标题', category: 'DEDUCTION' });
      prisma.subthread.findFirst.mockResolvedValue({ posts: [{ id: 'p1' }] });
      prisma.thread.update.mockResolvedValue({
  ...
  describe('子贴 (Subthread)', () => {
    it('创建子贴：含正文 → 创建子贴 + 第一楼 (floorNumber=1) + 发射事件', async () => {
      ...
      const result = await subthreadsService.create('t1', { title: '设定区', content: '正文' }, 'u1');
      expect(result!.id).toBe('s1');
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('post.created', expect.objectContaining({ isSubthreadBody: true }));
    });
    ...
    it('创建子贴：非管理权限 → 403', async () => {
      prisma.threadMember.findUnique.mockResolvedValue({ role: 'PARTICIPANT' });
      await expect(subthreadsService.create('t1', { title: '设定区' }, 'u2')).rejects.toThrow(BusinessException);
    });
    ...
    it('排序：首项非默认子贴 → 拒绝', async () => {
      ...
      await expect(subthreadsService.reorder('t1', ['b', 'a'], 'u1')).rejects.toThrow(BusinessException);
    });

    it('排序：含不存在子贴 → 拒绝', async () => {
      setupHelpers.mockThreadMember_ownerOrCollab(prisma);
      prisma.subthread.findMany.mockResolvedValue([{ id: 'a' }]);
      await expect(subthreadsService.reorder('t1', ['a', 'b'], 'u1')).rejects.toThrow(BusinessException);
    });
    ...
    it('排序：正常批量排 → 成功', async () => {
      ...
      prisma.subthread.findMany
        ...
        .mockResolvedValueOnce([{ id: 'a', title: 'sA', sortOrder: 0 }, { id: 'b', title: 'sB', sortOrder: 1 }, { id: 'c', title: 'sC', sortOrder: 2 }]);
      prisma.subthread.findFirst.mockResolvedValue({ id: 'a' });
      prisma.$transaction.mockImplementation(async (fn: any) => {
    ...
    it('删除：非默认子贴应设置 deletedAt', async () => {
      ...
      expect(prisma.subthread.update).toHaveBeenCalledWith(
        expect.objectContaining({ where: { deletedAt: null, id: 's2' }, data: { deletedAt: expect.any(Date) } }),
      );
    });
  });
  ...
  describe('楼层创建 (Post Create)', () => {
    ...
    it('创建楼中楼回复：floorNumber 应为 null，parentPostId 设置为父楼层', async () => {
      ...
      const txFn = (fn: any) => fn({
        ...basicTx(),
        post: {
          aggregate: jest.fn(),
          create: jest.fn().mockResolvedValue({ id: 'p2', floorNumber: null, parentPostId: 'p1', content: 'reply', author: { id: 'u2', username: 'user2' } }),
        },
      });
      prisma.$transaction.mockImplementation(txFn);

      const result = await postsService.create('s1', { content: 'reply', parentPostId: 'p1' }, 'u2');
    ...
    it('创建回复：replyToPostId 指定目标，不设置 floorNumber', async () => {
      ...
      prisma.$transaction.mockImplementation(async (fn: any) => fn({
        ...
        post: {
          ...
          create: jest.fn().mockResolvedValue({ id: 'p3', floorNumber: null, parentPostId: 'p_parent', replyToPostId: 'p_target', content: 'reply', author: { id: 'u2', username: 'user2' } }),
        },
      }));

      const result = await postsService.create('s1', { content: 'reply', parentPostId: 'p_parent', replyToPostId: 'p_target' }, 'u2');
      expect(result.floorNumber).toBeNull();
    });
    ...
    it('发帖到不存在的子贴 → 404', async () => {
      prisma.subthread.findUnique.mockResolvedValue(null);
      await expect(postsService.create('x', { content: 'test' }, 'u1')).rejects.toThrow(BusinessException);
    });
  });
  ...
  describe('发帖权限 (Posting Policy)', () => {
    it('COLLABORATORS：非协作者 → 403', async () => {
      ...
      prisma.threadMember.upsert.mockResolvedValue({});
      prisma.threadMember.findUnique.mockResolvedValue({ role: 'PARTICIPANT' });
      await expect(postsService.create('s1', { content: 'test' }, 'u2')).rejects.toThrow(BusinessException);
    });
    ...
    it('PLAYERS：非玩家 → 403', async () => {
      ...
      prisma.threadMember.upsert.mockResolvedValue({});
      prisma.threadMember.findUnique.mockResolvedValue({ role: 'PARTICIPANT', playerMarked: false });
      await expect(postsService.create('s1', { content: 'test' }, 'u2')).rejects.toThrow(BusinessException);
    });
  ...
  describe('事件发射 (Event Emission)', () => {
    it('已发布帖发楼层：触发 post.created 事件', async () => {
      const subthread = {
        id: 's1', threadId: 't1', title: '子贴A', postingPolicy: 'PARTICIPANTS' as const,
        thread: { published: true, title: '主题A' },
      };
      ...
      prisma.$transaction.mockImplementation(async (fn: any) => fn({
        ...
        post: {
          ...
          create: jest.fn().mockResolvedValue({ id: 'p11', floorNumber: 11, content: 'test', author: { id: 'u1', username: 'test' } }),
        },
      }));
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('post.created', expect.objectContaining({
        postId: 'p11', content: 'test', userId: 'u1', threadId: 't1', subthreadId: 's1', parentPostId: null,
      }));
    });
    ...
    it('未发布帖发楼层：不触发事件', async () => {
      ...
      prisma.$transaction.mockImplementation(async (fn: any) => fn({
        ...
        post: {
          ...
          create: jest.fn().mockResolvedValue({ id: 'p1', floorNumber: 1, content: 'test', author: { id: 'u1', username: 'test' } }),
        },
      }));
    ...
    it('已发布帖发楼中楼：parentPostId 非空时仍应触发事件', async () => {
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('post.created', expect.objectContaining({ parentPostId: 'p1' }));
    });
  });
  ...
  describe('帖子删除 (Post Delete)', () => {
    it('软删除非第一楼 → 成功', async () => {
      ...
      expect(prisma.post.update).toHaveBeenCalledWith(
        expect.objectContaining({ where: { deletedAt: null, id: 'p1' }, data: { deletedAt: expect.any(Date) } }),
      );
    });
...
prisma.post.findUnique.mockResolvedValue({ id: 'p1', authorId: 'u1', subthread: { deletedAt: null } });
  ...
  // ======================== 第九部分：点赞 ========================
  describe('点赞 (Like)', () => {
    it('点赞：upsert + likeCount +1', async () => {
      prisma.post.findUnique.mockResolvedValue({ id: 'p1', threadId: 't1', likeCount: 0 });
      prisma.postLike.upsert.mockResolvedValue({});
      ...
      expect(prisma.postLike.upsert).toHaveBeenCalledWith(
        expect.objectContaining({ where: { postId_userId: { postId: 'p1', userId: 'u1' } } }),
      );
      expect(prisma.post.update).toHaveBeenCalledWith({
    ...
    it('点赞：重复调用应幂等', async () => {
      ...
      prisma.postLike.upsert.mockResolvedValue({});
      prisma.post.update.mockResolvedValue({ id: 'p1', likeCount: 2 });
      await postsService.like('p1', 'u1');
      expect(prisma.postLike.upsert).toHaveBeenCalledTimes(1);
    });
    ...
    it('点赞不存在的帖子 → 404', async () => {
      prisma.post.findUnique.mockResolvedValue(null);
      await expect(postsService.like('x', 'u1')).rejects.toThrow(BusinessException);
    });
  });
  ...
  describe('帖子查询 (Post Query)', () => {
    ...
    it('findReplies：已软删子贴返回 404', async () => {
      prisma.post.findUnique.mockResolvedValue({ id: 'p1', threadId: 't1', subthread: { deletedAt: new Date() } });
  ...
  describe('草稿池 (Draft Pool)', () => {
    ...
    it('5 槽满返回错误', async () => {
      prisma.draft.findMany.mockResolvedValue([{ slot: 1 }, { slot: 2 }, { slot: 3 }, { slot: 4 }, { slot: 5 }]);
  ...
  describe('邀请链接 (Invite Link)', () => {
    it('私密已发布帖：正常生成', async () => {
      prisma.thread.findUnique.mockResolvedValue({ id: 't1', ownerId: 'u1', published: true, visibility: 'PRIVATE' });
      prisma.threadInvite.upsert.mockResolvedValue({ id: 'inv1', threadId: 't1', token: 'abc123def456gh78' });
      const result = await threadsService.createInviteLink('t1', 'u1');
      expect(result.token).toBeDefined();
    });

    it('公开帖：禁止生成', async () => {
      prisma.thread.findUnique.mockResolvedValue({ id: 't1', ownerId: 'u1', published: true, visibility: 'PUBLIC' });
    ...
    it('通过邀请链接加入：未发布帖拒绝', async () => {
      prisma.threadInvite.findUnique.mockResolvedValue({ threadId: 't1', thread: { id: 't1', visibility: 'PRIVATE', published: false } });
  ...
  describe('成员管理 (Thread Members)', () => {
    it('公开帖自由加入 → 成功', async () => {
      prisma.thread.findUnique.mockResolvedValue({ id: 't1', visibility: 'PUBLIC', published: true });
      prisma.threadMember.findUnique.mockResolvedValue(null);
      prisma.threadMember.create.mockResolvedValue({ id: 'm1' });
  ...
  describe('ThreadAccess 访问控制', () => {
    ...
    it('未发布帖非 owner → 404', async () => {
      prisma.thread.findUnique.mockResolvedValue({ visibility: 'PUBLIC', published: false, ownerId: 'u1' });
    ...
    it('私密帖非成员 → 404', async () => {
      prisma.thread.findUnique.mockResolvedValue({ visibility: 'PRIVATE', published: true, ownerId: 'u1' });
      prisma.threadMember.findUnique.mockResolvedValue(null);
      await expect(postsService.findAllBySubthread('s1', 'u2')).rejects.toThrow(BusinessException);
    });
