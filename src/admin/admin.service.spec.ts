...
describe('AdminService', () => {
  ...
  it('显式接收者优先于条件且始终排除注销用户', () => {
    expect(service.buildRecipientWhere({
      recipientIds: ['user-1'],
      conditions: { emailVerified: true },
    } as SendSystemNotificationDto)).toEqual({
      id: { in: ['user-1'] },
      deletedAt: null,
    });
  });
  ...
  it('没有接收者时不入队也不写审计日志', async () => {
    ...
    await expect(service.sendSystemNotification(
  ...
  it('按稳定游标分批发送并用同一事件键写审计日志', async () => {
    ...
    await expect(service.sendSystemNotification(
      { content, payload: { kind: 'maintenance' }, threadId: 'thread-1' } as SendSystemNotificationDto,
      'admin-1',
      '127.0.0.1',
    ...
    expect(prisma.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        ...
        ip: '127.0.0.1',
        detail: expect.stringContaining(`"content":"${'系'.repeat(200)}"`),
      }),
    });
  });
